...
import { loadHostPaymentRecords, loadRoomAdhocSlots, textslotsToText, yymmdd, createAdhoc, deleteAdhoc, message, alert } from '../utils';
import {
  Headline,
  TextInput,
  Button,
...
const Screen = props => {
  ...
  function getTime(slots) {
    return textslotsToText(Object.keys(slots || {}).join(',')).replace(/-/g, '\n-\n');
  }
  ...
  const fullSlotsStr = '["00:00","00:30","01:00","01:30","02:00","02:30","03:00","03:30","04:00","04:30","05:00","05:30","06:00","06:30","07:00","07:30","08:00","08:30","09:00","09:30","10:00","10:30","11:00","11:30","12:00","12:30","13:00","13:30","14:00","14:30","15:00","15:30","16:00","16:30","17:00","17:30","18:00","18:30","19:00","19:30","20:00","20:30","21:00","21:30","22:00","22:30","23:00","23:30"]';
  ...
  const renderItem = (x) => {
    if (x.isOpen == null) {
    return (
      <View style={{ flex: 1, flexGrow: 0, flexDirection: 'row', alignItems: 'center', minHeight: 80, justifyContent: 'space-between', padding: 8, margin: 0, borderColor: 'gray', borderBottomWidth: 1 }}>
        <View style={{...style.timeslot}}><Text style={{textAlign: 'center'}}>{getTime(x.slots)}</Text></View>
        <Text style={{flex: 1, flexGrow: 1}}>{'房間：' + x.room + '\n' + '租客：' + x.user}</Text>
        <Button style={{flex: 1, flexGrow: 0}} onPress={() => { chatWith(x); }}><Icon size={30} name={Platform.OS === 'ios'?'ios-chatbubbles':'md-chatbubbles'} style={{ color: Colors.main }}/></Button>
      </View>
      );
    }
    else {
      return (
        <View style={{ flex: 1, flexGrow: 0, flexDirection: 'row', alignItems: 'center', minHeight: 80, justifyContent: 'flex-start', padding: 8, margin: 0, borderColor: 'gray', borderBottomWidth: 1 }}>
          <View style={{...style.timeslot}}><Text style={{textAlign: 'center'}}>{getTime(x.slots)}</Text></View>
          <Text style={{flex: 1, flexGrow: 1}}>{x.room + ' 臨時' + (x.isOpen ? '開放' : '休息') + (x.isOpen ? ' 時租$' + x.price : '')}</Text>
          <Button style={{flex: 1, flexGrow: 0}} onPress={() => { deleteAdhoc(x.roomId, x.id); reload(); }}><Icon size={30} name={Platform.OS === 'ios'?'ios-close-circle':'md-close-circle'} style={{ color: 'red' }}/></Button>
        </View>
      )
    }
  }
    const renderEmptyDate = () => {
      return (
      <View><Text style={{width: '100%', marginLeft: 20, marginTop: 40}}>沒有預約</Text></View>
    );
  }
    ...
    <View style={styles.container}>
      <View style={style.header}>
        <View style={styles.row}>
          <Text style={{color: Colors.bg, fontSize: 20, lineHeight: 60, marginLeft: 30}}>日曆</Text>
          <View style={{flexDirection:'row', alignItems: 'flex-end'}}>
            <Button onPress={reload}><Icon
              size={24}
              name={
                Platform.OS === 'ios'
                  ? 'ios-refresh'
            ...
            <Button style={style.addButton} onPress={addAdhoc}><Icon
              size={24}
              name={
                Platform.OS === 'ios'
                  ? 'ios-add-circle'
          ...
          <ActionSheet
            ...
            onPress={(index) => {
              if (index < startSlots.length) {
                ...
                setEndSlots(JSON.parse(fullSlotsStr).filter(x => x > startSlots[index]));
              }
            }}
          />
          <TouchableOpacity>
          ...
          <ActionSheet
            ...
            onPress={(index) => {
              if (index < endSlots.length) {
                ...
                setStartSlots(JSON.parse(fullSlotsStr).filter(x => x < endSlots[index]));
              }
            }}
          />
          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#E7E7E7' }}>
            <Input
              style={{ flexGrow: 1 }}
              name="open"
              binding={binding}
              editable={false}
            />
            <View style={{ paddingTop: 12, alignItems: 'center', borderBottomColor: '#B7B7B7', height: '100%', borderBottomWidth: 1 }}>
              <Switch
                trackColor={{ false: 'grey' }}
                style={{ marginRight: 8, borderBottomWidth: 2, borderBottomColor: '#C9C9C9', marginBottom: 2 }}
                onValueChange={setIsOpen}
                value={isOpen} />
            </View>
          </View>
          <Input name="price" style={isOpen ? {} : { display: 'none' }} binding={binding} />
          <Button onPress={() => {
            confirmAddAdhoc();
          }}>確定</Button>
          <Button onPress={() => {

...
const Screen = props => {
  ...
  function onConfirm() {
    ...
    const lastAction = () => {
      ...
      action.then(() => {
        notifyRoomCreate(roomRef.id).then(x => {
          if (x.msg != true) {
            Alert.alert('發生錯誤');
          }
        });
  ...
  return (
    <View style="styles.container">
      ...
      <View style={style.imageGroup}>
        <Image key={'photo1' + state.photo1} source={{ uri: state.photo1 }} direct={true} style={{ width: 60, height: 60 }} />
        <Image key={'photo2' + state.photo2} source={{ uri: state.photo2 }} direct={true} style={{ width: 60, height: 60 }} />
        <Image key={'photo3' + state.photo3} source={{ uri: state.photo3 }} direct={true} style={{ width: 60, height: 60 }} />
        <Image key={'photo4' + state.photo4} source={{ uri: state.photo4 }} direct={true} style={{ width: 60, height: 60 }} />
        <Image key={'photo5' + state.photo5} source={{ uri: state.photo5 }} direct={true} style={{ width: 60, height: 60 }} />
      </View>
        ...
        <View style={style.infoBox}>
          ...
          <Text style={{...style.fieldText}}>{(state.facilities || []).join(',')}</Text>
